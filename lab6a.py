...
class Student:
    ...
    def __init__(self, name, number):
        self.name = name
        self.number = str(number) #force self.number to be stored as a string
        self.courses = {}
    ...
    def displayGPA(self):
        gpa = 0.0
        if len(self.courses) == 0: #Error handling if the student does not have any courses with assigned marks
    ...
    def displayCourses(self):
        passed_courses = []
        for course in self.courses.keys():
            if self.courses[course]: #if the course has a value that is not None / 0, append it to the list of passed courses
